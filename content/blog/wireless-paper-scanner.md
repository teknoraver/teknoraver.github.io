...
That meant two things: `scanimage` had to output a *compressed* format, and compression had to happen while scanning, not after.  
...
So I got the same out-of-memory failure as before, just a bit later.
...
I had to run `saned` (the SANE network daemon) on the router to access the scanner,
