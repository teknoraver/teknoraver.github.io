...
tags = ["hacking", "reverse-engineering", "cryptography", "mips", "embedded"]
+++
...
I compiled an OpenWrt initramfs image and loaded it via tftp:
...
This is a common practice in resource-constrained systems, most notably in the BusyBox project.

I started IDA to look at this binary; clearly, `httpConfUpload` was the function to start hacking from.

{{< figure src="ida.png" >}}

Given a reference to `des_min_do` and some strings starting with *DES_*, I suspected that *DES* was the cipher used to encrypt the file.  
Furthermore, it had a lot of bitwise operators and loops, common for cryptographic functions:
...
It could be some salt or key passed to the encryption function as an argument, so I noted the string: **478DA50BF9E3D2CF**.

{{< figure src="key.png" >}}

I tried to decrypt it with mdecrypt using that string as the key, but without success:
...
when I calculate it with my hex editor, it matches:
