...
I filtered the running processes and found `sleep inf` running, so I discovered that `sleep` supports the "inf" argument to sleep indefinitely.  
This was not documented in the man page, so I looked at the [sleep source code](https://github.com/coreutils/coreutils/blob/master/src/sleep.c) to learn more,
...
This is a very smart way to sleep indefinitely, instead of using a loop.

But why was `ppoll` being called instead of `pause`? Obviously, the result is the same, but why not just call `pause` directly?  
So I wrote this small tool to see which syscall was actually being called:
```c
#include <unistd.h>
...
The answer: it depends on the architecture. This happens on an ARM machine:
```text
