...
> **TL;DR**: GCC and Clang support `__attribute__((cleanup(fn)))`, which calls `fn(&var)` automatically when `var` goes out of scope — effectively RAII for C. No more `goto out`-ladder error cleanup, no more forgotten `free`/`close`. Used heavily in systemd and (since 2023) the Linux kernel.

When I started contributing to systemd, I discovered a new C feature I wouldn't want
to live without anymore: cleanup attributes.  
...
As the function grows longer, the error paths become more and more complex,
...
**Cons**
* Repetition hell: the same three cleanup calls in four different places.
...
* Conciseness: 50% smaller than the traditional version
...
### Returning pointers
You might ask: what if I need to return a pointer to a resource that should be cleaned up later?  
Once the cleanup attribute is applied, the cleanup function is bound to the variable and it's
impossible to skip the cleanup call. So how do we return a valid resource?  
...
```c
// Cleanup for temporary files — unlinks the file and frees the malloc'd path.
// Expects *path to be heap-allocated (e.g. via strdup or malloc + snprintf).
