...
> _Benchmarked against Linux 4.19 and FreeBSD 11.2; numbers will differ on newer kernels._
...
## Test setup
To avoid having to set up cables and interfaces on bare metal systems, I decided to get a single,
...
and the system was booted with `nosmt=force` to avoid using their HT siblings.
...
The idea makes sense, though I didn't find a way to do the same under Linux.
...
At first glance, FreeBSD performance looked disastrous, so I shared my results with the #freebsd folks on IRC, and they told me that slow VirtIO drivers for FreeBSD are a known issue. Not a big deal then, KVM was developed on Linux and I'm sure the Linux guest drivers are more optimized.
I changed the VM setup to use an emulated Intel Gigabit ethernet or Realtek rtl8139, but I got very poor results from both OSes, so I won't even report them.
So I ran the test again on the loopback interface on both guests. It's not a very professional test, but at least it avoids any VirtIO deficiency.
To make the test fair, I lowered the Linux MTU to 16384, as this is the maximum allowed on the FreeBSD loopback device.
...
Xeon processors from Broadwell onward have a feature named [Posted Interrupts](https://software.intel.com/sites/default/files/managed/c5/15/vt-directed-io-spec.pdf).
This feature allows a guest OS to receive interrupts from a device without passing through the hypervisor. [KVM has supported this since 2013](https://git.kernel.org/linus/5a71785dde307f6ac80e83c0ad3fd694912010a1), and thanks to this, the performance of a device passed via PCI passthrough is identical to bare metal:
...
So I got some Intel 82599ES 10 Gigabit cards as suggested by the [FreeBSD Network Performance Tuning](https://wiki.freebsd.org/NetworkPerformanceTuning) wiki, plus another identical server, and connected the two back to back with this setup.
...
The server was running a single Linux or FreeBSD machine at once with PCIe passthrough, while another server was running TRex bound to two 10 Gbit cards.
[TRex](https://trex-tgn.cisco.com/) is a powerful DPDK-based traffic generator capable of generating tens of millions of packets per second. It can easily achieve line rate with 10 Gbit cards by sending 64-byte frames from 4 CPUs.
TRex sends 10 Gbit (14.8 million 64-byte packets) per second from NIC1; the packets are received from NIC2, handled by the OS under test (which sends them to NIC3), and finally go back to TRex, which checks them and produces statistics.
With this setup and PCI passthrough, test results became more stable and reproducible between different runs, so I assumed that this was the correct way to do it.
...
The first test is a software bridge. The two interfaces are bridged and packets are just forwarded.

{{< figure src="bridge.png" caption="L2 forwarding" >}}

The first thing that struck me is that the FreeBSD packet rate was substantially the same with one or 8 CPUs.
I investigated a bit and found it to be a known issue: bridging under FreeBSD is known to be slow because
the *if_bridge* driver is practically single-threaded due to excessive locking, as documented in the [FreeBSD network optimization guide](https://wiki.freebsd.org/NetworkPerformanceTuning).

The second thing I noted is that when running a test on a single-core FreeBSD guest,
the system froze until traffic was stopped. This only happened to FreeBSD when the guest had only one core.  
Initially I thought it could be a glitch of the serial or tty driver,
but then I ran a `while sleep 1; do date; done` loop, and if it had been just an output issue the time wouldn't have frozen.
I looked through all the sysctls to check if the FreeBSD kernel was preemptible, and it was, so I couldn't explain what was going on.  
...
The second test is routing. Two IP addresses belonging to different networks are assigned to the interfaces, and the TRex NIC4 address is set as the default route. TRex is sending packets to the first interface and packets are forwarded.
...
The third test is firewalling. The setup is the same as the routing test, except that some firewall rules are loaded.
The rules are generated in a way that they can't match any packets sent from TRex (different port range than the generated traffic); they're there only to add weight.
Both OSes have two firewall systems: Linux has iptables and nftables, while FreeBSD has PF and IPFW. I tested all of them, and the graph below reports the performance of iptables and IPFW, which proved faster than the other two solutions.

{{< figure src="firewall.png" caption="packet filtering" >}}

As mentioned earlier, I deliberately omitted the nftables and PF numbers to avoid confusion. If you want to see all the numbers, here is the [raw data](https://docs.google.com/spreadsheets/d/e/2PACX-1vQqhCYLDEyngnWJAcBV4xahScYXt_edKL-HDxzsdIjuTIdg8LOdYZU1hvcwklJ_Np1aaLmP0fzrzOz5/pubhtml?gid=1147998389&single=true).
...
FreeBSD scales relatively well with core numbers (except in bridge mode, which is effectively single-threaded), but Linux does a near-perfect job using all the power of a multicore system. The same applies to firewalling, where we can see that a large firewall ruleset can disrupt the performance of both kernels, unless you use tricks like fastpath and hardware offloading.
